...
#HW code
```{r}
_________(lm_fit, ________ = Boston)
```
...
```{r}
bind_cols(
  predict(lm_fit, new_data = __________),
  Boston
) %>%
  select(_____, .pred)
```
...
#HW code
```{r}
_________(lm_fit, new_data = Boston)
```
...
#HW code
```{r}
rec_spec <- _______(medv ~ ., data = Boston) %>%
  __________interact(~ ______:_______)
```
